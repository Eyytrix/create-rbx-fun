...
--[[
	...
	Table.Copy(Table tbl)
	Table.CopyShallow(Table tbl)
	Table.Sync(Table tbl, Table templateTbl)
	Table.Print(Table tbl, String label, Boolean deepPrint)
	Table.FastRemove(Table tbl, Number index)
	Table.FastRemoveFirstValue(Table tbl, Variant value)
	Table.Map(Table tbl, Function callback)
	Table.Filter(Table tbl, Function callback)
	Table.Reduce(Table tbl, Function callback [, Number initialValue])
	Table.Assign(Table target, ...Table sources)
	Table.IndexOf(Table tbl, Variant item)
	Table.Reverse(Table tbl)
	Table.Shuffle(Table tbl)
	Table.IsEmpty(Table tbl)
	Table.EncodeJSON(Table tbl)
	Table.DecodeJSON(String json)
	...
	EXAMPLES:
		...
		CopyShallow:
			...
			local tbl = {"a", "b", "c"}
			local tblCopy = Table.CopyShallow(tbl)
		...
		Sync:
			...
			local tbl2 = {points = 0}
			Table.Sync(tbl2, tbl1)  -- In words: "Synchronize table2 to table1"
			print(tbl2.deaths)
		...
		FastRemove:
			...
			Table.FastRemove(tbl, 2)   -- Remove "there" in the array
		...
		FastRemoveFirstValue:
			...
			local removed, atIndex = Table.FastRemoveFirstValue(tbl, "hello")
			if (removed) then
		...
		Map:
			...
			local people = Table.Map(peopleData, function(item)
		...
		Filter:
			...
			local peopleUnderForty = Table.Filter(people, function(item)
				return item.Age < 40
			end)
		...
		Reduce:
			...
			local tblSum = Table.Reduce(tbl, function(accumulator, value)
		...
		Assign:
			...
			local function CreateCar()
				...
				return Table.Assign({}, Driver(), Teleporter())
			end
		...
		IndexOf:
			...
			local abcIndex = Table.IndexOf(tbl, "abc")     -- > 4
			local helloIndex = Table.IndexOf(tbl, "Hello") -- > 1
			local numberIndex = Table.IndexOf(tbl, 64)     -- > nil
		...
		Reverse:
			...
			local tbl = {2, 4, 6, 8}
			local rblReversed = Table.Reverse(tbl)  -- > {8, 6, 4, 2}
...
local function Print(tbl, label, deepPrint)
	...
	assert(
		...
		"Second argument must be a string or nil"
	)
	...
	local function PrintTable(t, lvl, lbl)
		...
		for _, v in ipairs(nonTbls) do
			Insert(
				tostring(v.k) .. ":" .. (" "):rep(keySpaces - #tostring(v.k)) .. v.v,
				lvl
			)
		end
		if (deepPrint) then
...
Table.Sync = Sync
Table.FastRemove = FastRemove
Table.FastRemoveFirstValue = FastRemoveFirstValue
Table.Print = Print
Table.Map = Map
Table.Filter = Filter
Table.Reduce = Reduce
